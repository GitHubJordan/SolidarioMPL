...
from django.contrib.auth import login as auth_login, authenticate, logout  # type: ignore
...
def items_for_sale(request):
    ...
    if request.method == "POST":
        ...
        if item.estado == "aprovado":  # Somente itens aprovados podem ser comprados
            ...
            messages.error(
                request, f"O item {item.name} não está disponível para compra."
            )
...
def not_donor(request):
    return render(
        request, "donations/not_donor.html", {"username": request.user.username}
    )
...
def item_donation_view(request):
    ...
    if request.user.is_superuser:
        items_in_analysis = Item.objects.filter(
            estado="analise"
        )  # Apenas itens em análise
    else:
        items_in_analysis = Item.objects.filter(
            donated_by__user=request.user, estado="analise"
        )
...
def validate_item(request, item_id):
    ...
    return redirect(
        "register_item"
    )  # Redirecionar de volta para a página de doação de itens
...
def edit_profile_view(request):
    ...
    if request.method == "POST":
        form = DonorForm(
            request.POST, request.FILES, instance=donor, user_instance=request.user
        )
        if form.is_valid():
            form.save()
